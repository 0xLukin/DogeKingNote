...
interface DividendPayingTokenInterface {
    /// @notice View the amount of dividend in wei that an address can withdraw.  查看地址可以提取的股息金额
    /// @param _owner The address of a token holder.
    /// @return The amount of dividend in wei that `_owner` can withdraw.
    function dividendOf(address _owner) external view returns (uint256);

    /// @notice Withdraws the ether distributed to the sender.  提取股息金额
    /// @dev SHOULD transfer `dividendOf(msg.sender)` wei to `msg.sender`, and `dividendOf(msg.sender)` SHOULD be 0 after the transfer.
    ///  MUST emit a `DividendWithdrawn` event if the amount of ether transferred is greater than 0.
    function withdrawDividend() external;

    /// @dev This event MUST emit when ether is distributed to token holders. 分发股息的事件
    /// @param from The address which sends ether to this contract.
    /// @param weiAmount The amount of distributed ether in wei.
    event DividendsDistributed(address indexed from, uint256 weiAmount);

    /// @dev This event MUST emit when an address withdraws their dividend. 提取股息的事件
...
interface DividendPayingTokenOptionalInterface {
    /// @notice View the amount of dividend in wei that an address can withdraw.  是否可提取股息
    /// @param _owner The address of a token holder.
    /// @return The amount of dividend in wei that `_owner` can withdraw.
    ...
    /// @notice View the amount of dividend in wei that an address has withdrawn.  查看地址提取的股息金额
    /// @param _owner The address of a token holder.
    ...
    /// @notice View the amount of dividend in wei that an address has earned in total.  查看一个地址总共获得的股息金额
    ...
    DividendPayingTokenOptionalInterface
{
    ...
    //有了“数量”，即使收到的乙醚量很小，我们也可以适当分配股息。
    // For more discussion about choosing the value of `magnitude`,
    //  see https://github.com/ethereum/EIPs/issues/1726#issuecomment-472352728
    uint256 internal constant magnitude = 2**128;

    uint256 internal magnifiedDividendPerShare; // 放大每股股息

    // 关于除数校正:
    // 如果“_user”的令牌余额从未改变，则可以使用以下公式计算“_user”的红利:
    //   `dividendOf(_user) = dividendPerShare * balanceOf(_user)`.
    // 当“balanceOf（_user）”被改变（通过铸造/燃烧/转移令牌）时,
    // 不应更改“dividendOf（_user）,
    // 但是“dividendPerShare*balanceOf（_user）”的计算值被改变.
    // 为了保持“dividedOf（_user）”不变，我们添加了一个校正项:
    //   `dividendOf(_user) = dividendPerShare * balanceOf(_user) + dividendCorrectionOf(_user)`,
    // 其中每当“balanceOf（_user）”更改时，“dividedCorrectOf（_user）”都会更新:
    //   `dividendCorrectionOf(_user) = dividendPerShare * (old balanceOf(_user)) - (new balanceOf(_user))`.
    // 因此，现在“dividendOf（_user）”在“balanceOf（_user）”更改前后返回相同的值.  也就是说会添加一个参数去动态固定已经分配好的股息，然后不影响后面分配的计算
    mapping(address => int256) internal magnifiedDividendCorrections;
    mapping(address => uint256) internal withdrawnDividends;

    uint256 public totalDividendsDistributed; // 分配的股息总额
...
contract BABYTOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    ...
    uint256 public lastProcessedIndex; //上一次处理的索引

    mapping(address => bool) public excludedFromDividends; // 不包括股息

    mapping(address => uint256) public lastClaimTimes; // 上次索赔时间

    uint256 public claimWait;
    uint256 public minimumTokenBalanceForDividends; // 股息的最小代币余额
    ...
    function withdrawDividend() public pure override {
        ...
        // “Dividend_Tracker：已禁用提取股息。请在主BABYTOKEN合约上使用'索赔'功能。”
    }
    
    // 不分配股息的地址，保留当前股息余额，清零计算股息，从分红持有者中移除
    ...
    function getAccount(address _account)
        ...
        returns (
            address account,
            int256 index,
            int256 iterationsUntilProcessed, //迭代直到处理
            uint256 withdrawableDividends,
            uint256 totalDividends,
            uint256 lastClaimTime,
            uint256 nextClaimTime,
            uint256 secondsUntilAutoClaimAvailable //到下一个自动分红的秒数
        )
    {
        account = _account;
...
contract BABYTOKEN is ERC20, Ownable, BaseToken {
    ...
    constructor(
        ...
        uint256 minimumTokenBalanceForDividends_,
        address serviceFeeReceiver_, // 服务费接收人_
        uint256 serviceFee_ // 服务费
        ...
        */
        ...
        payable(serviceFeeReceiver_).transfer(serviceFee_); //服务费
    }
    ...
    function _transfer(
        ...
        bool canSwap = contractTokenBalance >= swapTokensAtAmount; //代币要大于总量的0.002%才能
